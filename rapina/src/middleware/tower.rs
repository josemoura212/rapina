...
///
/// On each request the layer is applied to a fresh [`NextService`] that
/// delegates to the remaining Rapina middleware chain. The resulting tower
/// service is then called to process the request.
///
/// # Body type
///
...
///     .await
/// ```
pub struct TowerLayerMiddleware<L> {
    layer: L,
}
...
impl<L: fmt::Debug> fmt::Debug for TowerLayerMiddleware<L> {
